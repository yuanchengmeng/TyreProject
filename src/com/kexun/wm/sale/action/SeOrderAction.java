...
public class SeOrderAction {
	...
	private File file;
	private SeOrder seOrder = new SeOrder();
...
//		seOrder=new SeOrder();
		...
		for(int i=0;i<seOrderEntryList.size();i++){
			seOrderEntryList.get(i).setAllBarcode(seOrderService.getAllBarcode(seOrder, seOrderEntryList.get(i)));
		}
		
		vehicleList = seOrderService.queryVehicleByOrderId(id);
