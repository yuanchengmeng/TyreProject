...
public class SeOrderDaoImpl implements SeOrderDao {
	...
	public int getAllBarcode(SeOrder seOrder, SeOrderEntry seOrderEntry) throws Exception {
		...
		try {
			session = fy.openSession();
			String hql = "select count(*) from hand_store where StoreState='已出库' and OrderID="
						+seOrderEntry.getSeOrder().getFinterID()+" and ProductID="+seOrderEntry.getProduct().getFItemID();
			if(null != seOrder && seOrder.getCarId() >0){
				hql += " and vehicleID="+seOrder.getCarId();
			}
			if(null != seOrder && StringUtils.isNotBlank(seOrder.getOutMan())){
				hql += " and OutMan like '%"+seOrder.getOutMan()+"%'";
			}
 			Query q = session.createSQLQuery(hql);
 			result = (Integer) q.uniqueResult();
		} catch (HibernateException hi) {
