...
<script type="text/javascript">
	function page(flag) {
		...
		if ("first" == flag) {
		location.href = "/TyreProject/store/queryStoreReport?flag=2&&pageNo=1"
					+ "&&pageSize=" + pageSize+"&&storeReport.timeStart="+timeStart+"&&storeReport.timeEnd="+timeEnd+"&&classId="+classId;
		} else if ("end" == flag) {
		location.href = "/TyreProject/store/queryStoreReport?flag=2&&pageNo=" + pageCount
					+ "&&pageSize=" + pageSize+"&&storeReport.timeStart="+timeStart+"&&storeReport.timeEnd="+timeEnd+"&&classId="+classId;
		} else if ("last" == flag) {
		location.href = "/TyreProject/store/lastStoreReportPage?flag=2&&pageNo=" + pageNo
					+ "&&pageSize=" + pageSize+"&&storeReport.timeStart="+timeStart+"&&storeReport.timeEnd="+timeEnd+"&&classId="+classId;
		} else if ("next" == flag) {
		location.href = "/TyreProject/store/nextStoreReportPage?flag=2&&pageNo=" + pageNo
					+ "&&pageSize=" + pageSize+"&&storeReport.timeStart="+timeStart+"&&storeReport.timeEnd="+timeEnd+"&&classId="+classId;
		} else if ("go" == flag) {
		   if (parseInt(gotopage) <= parseInt(pageCount)){
				location.href = "/TyreProject/store/queryStoreReport?flag=2&&pageNo="
		...
		location.href = "/TyreProject/store/queryStoreReport?flag=1&&pageNo=1"
					+ "&&pageSize=" + pageSize+"&&storeReport.timeStart="+timeStart+"&&storeReport.timeEnd="+timeEnd+"&&classId="+classId;
		}
	}
	...
	function exportIn() {
	    ...
	    var timeEnd = document.getElementById("timeEnd").value;
	    var classId = document.getElementById("classId").value;
		location.href = "/TyreProject/store/exportStoreReportExcel?flag=2&&storeReport.timeStart="+timeStart+"&&storeReport.timeEnd="+timeEnd+"&&classId="+classId;
	 }
												...
												<tbody>
													<tr>
														...
														<td
															...
															class="dqwz"><b>当前位置：</b> 轮胎出库库统计</span></td>
													...
													<tr>
														...
														<td><br>
															<table width="100%" border="0" cellpadding="0"
																cellspacing="1">
																<tr class="query_two">
																	<td width="50%">&nbsp;出库时间：<input type="text"
																	...
																	<td>出库班次：<select id="classId" name="classId">
