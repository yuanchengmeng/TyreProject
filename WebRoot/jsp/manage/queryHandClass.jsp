...
<script type="text/javascript">
	function getClassItem() {
		...
		location.href = "/TyreProject/manage/getHandClassItem?pageNo=" + pageNo
	...
	function modifyCheckBox() {
		...
		if (count == 1) {
			location.href = "/TyreProject/manage/getHandClassById?handClass.id="
	...
	function deleteClass() {
		...
		if (count == 0) {
			...
			if (window.confirm('你确定要删除吗？')) {
				location.href = "/TyreProject/manage/deleteHandClass?ids=" + value
	...
	function page(flag) {
		...
		if ("first" == flag) {
			location.href = "/TyreProject/manage/queryHandClass?pageNo=1"
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("end" == flag) {
			location.href = "/TyreProject/manage/queryHandClass?pageNo=" + pageCount
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("last" == flag) {
			location.href = "/TyreProject/manage/lastHandClassPage?pageNo=" + pageNo
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("next" == flag) {
			location.href = "/TyreProject/manage/nextHandClassPage?pageNo=" + pageNo
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("go" == flag) {
			if (gotopage == "" || gotopage > pageCount) {
				alert("请输入不大于" + pageCount + "的正整数");
			} else {
				location.href = "/TyreProject/manage/queryHandClass?pageNo="
...
/* 	function queryClass() {
		...
		var pageSize = document.getElementById("pageSize").value;
		var pageNo = document.getElementById("pageNo").value;
		location.href = "/TyreProject/manage/queryHandClass?pageSize=" + pageSize
...
<body>
	${success}
	<s:form action="queryHandClass" namespace="/manage" enctype="multipart/form-data" theme="simple">
												...
												<tbody>
													...
													<tr>
														...
														<td><br style="line-height:3px">名称:<input
															name="handClass.name" id="nameId" value="${handClass.name}"
															...
															<table id="list" cellpadding="0" class="table"
																...
																<thead>
																	<tr>
																		...
																		<th style="color:black">班组名称</th>
																		<th style="color:black">班组描述</th>
																	</tr>
																</thead>
																<tbody>
																	<s:iterator value="handClassList" var="handClass">
																		<tr class="even">
																			<td style="width:30px;text-align:center"><input
																				type="checkBox" name="ids"
																				value="<s:property
																					value='#handClass.getId()' />"
																				style="border: none">
																			</td>
																			<td><s:property value="#handClass.getName()" /> 
																			</td>
																			<td><s:property value="#handClass.getDescription()" />
																			</td>
																		</tr>
																	</s:iterator>
																</tbody>
