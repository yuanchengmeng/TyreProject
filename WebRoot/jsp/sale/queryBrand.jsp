...
<script type="text/javascript">
	function getClassItem() {
		...
		location.href = "/TyreProject/sale/getBrandItem?pageNo=" + pageNo
	...
	function modifyCheckBox() {
		...
		if (count == 1) {
			location.href = "/TyreProject/sale/getBrandById?brand.id="
	...
	function deleteClass() {
		...
		if (count == 0) {
			...
			if (window.confirm('你确定要删除吗？')) {
				location.href = "/TyreProject/sale/deleteBrand?ids=" + value
	...
	function page(flag) {
		...
		if ("first" == flag) {
			location.href = "/TyreProject/sale/queryBrand?pageNo=1"
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("end" == flag) {
			location.href = "/TyreProject/sale/queryBrand?pageNo=" + pageCount
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("last" == flag) {
			location.href = "/TyreProject/sale/lastBrandPage?pageNo=" + pageNo
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("next" == flag) {
			location.href = "/TyreProject/sale/nextBrandPage?pageNo=" + pageNo
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("go" == flag) {
			if (gotopage == "" || gotopage > pageCount) {
				alert("请输入不大于" + pageCount + "的正整数");
			} else {
				location.href = "/TyreProject/sale/queryBrand?pageNo="
...
<body>
	${success}
	<s:form action="queryBrand" namespace="/sale" enctype="multipart/form-data" theme="simple">
												...
												<tbody>
													...
													<tr>
														...
														<td><br style="line-height:3px">名称:<input
															name="brand.name" id="nameId" value="${brand.name}"
															...
															<table id="list" cellpadding="0" class="table"
																...
																<thead>
																	<tr>
																		...
																		<th style="color:black">品牌名称</th>
																		<th style="color:black">品牌代码</th>
																...
																<tbody>
																	<s:iterator value="brandList" var="brand">
																		<tr class="even">
																			...
																			<td><s:property value="#brand.getName()" /> 
																			</td>
																			<td><s:property value="#brand.getCode()" />
																			</td>
																			<td><s:property value="#brand.getCtime()" /> 
																			</td>
																			<td><s:property value="#brand.getCuser()" />
																			</td>
																		</tr>
																	</s:iterator>
																</tbody>
