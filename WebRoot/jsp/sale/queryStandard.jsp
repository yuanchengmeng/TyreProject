...
<script type="text/javascript">
	function getClassItem() {
		...
		location.href = "/TyreProject/sale/getStandardItem?pageNo=" + pageNo
	...
	function modifyCheckBox() {
		...
		if (count == 1) {
			location.href = "/TyreProject/sale/getStandardById?standard.id="
	...
	function deleteClass() {
		...
		if (count == 0) {
			...
			if (window.confirm('你确定要删除吗？')) {
				location.href = "/TyreProject/sale/deleteStandard?ids=" + value
	...
	function page(flag) {
		...
		if ("first" == flag) {
			location.href = "/TyreProject/sale/queryStandard?pageNo=1"
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("end" == flag) {
			location.href = "/TyreProject/sale/queryStandard?pageNo=" + pageCount
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("last" == flag) {
			location.href = "/TyreProject/sale/lastStandardPage?pageNo=" + pageNo
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("next" == flag) {
			location.href = "/TyreProject/sale/nextStandardPage?pageNo=" + pageNo
					+ "&&pageSize=" + pageSize + "&&nameId=" + nameId;
		} else if ("go" == flag) {
			if (gotopage == "" || gotopage > pageCount) {
				alert("请输入不大于" + pageCount + "的正整数");
			} else {
				location.href = "/TyreProject/sale/queryStandard?pageNo="
...
<body>
	${success}
	<s:form action="queryStandard" namespace="/sale" enctype="multipart/form-data" theme="simple">
												...
												<tbody>
													...
													<tr>
														...
														<td><br style="line-height:3px">名称:<input
															name="standard.name" id="nameId" value="${standard.name}"
															...
															<table id="list" cellpadding="0" class="table"
																...
																<thead>
																	<tr>
																		...
																		<th style="color:black">规格名称</th>
																		<th style="color:black">速度级别</th>
																		<th style="color:black">花纹</th>
																		<th style="color:black">规格代码</th>
																		<th style="color:black">添加时间</th>
																		<th style="color:black">添加人</th>
																	</tr>
																</thead>
																<tbody>
																	<s:iterator value="standardList" var="standard">
																		<tr class="even">
																			<td style="width:30px;text-align:center"><input
																				...
																				value="<s:property
																					value='#standard.getId()' />"
																				style="border: none">
																			</td>
																			<td><s:property value="#standard.getName()" /> 
																			</td>
																			<td><s:property value="#standard.getSpeed()" /> 
																			</td>
																			<td><s:property value="#standard.getTread()" />
																			</td>
																			<td><s:property value="#standard.getCode()" />
																			</td>
																			<td><s:property value="#standard.getCtime()" /> 
																			</td>
																			<td><s:property value="#standard.getCuser()" />
																			</td>
																		</tr>
																	</s:iterator>
																</tbody>
